...
def calculate_parts(so: list,
    ...
    Важно: никакого округления вниз (Math.floor / int / |0).
    Дробная часть сохраняется — в пользу ученика.
    """
...
def calculate_final(total_so: float, total_sor: float, total_soch: float) -> float:
    ...
    Результат НЕ округляется вниз — дробная часть сохраняется.
    84.4 → 84.4, 79.7 → 79.7, 91.2 → 91.2
